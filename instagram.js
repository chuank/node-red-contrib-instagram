 ...
 * 2019-2022 Chuan Khoo.
 * www.chuank.com
 *
 * Rewritten from deprecated source from node-red-node-instagram
 * 2022: replace deprecated request with got & fix IG access tokens
 *
...
module.exports = function (RED) {
	...
	function refreshLongLivedAccessToken(node) {
		...
		node.log(
			'refreshLongLivedAccessToken time left: ' + numDays.toFixed(2) + ' days'
		);
	...
	function retrieveIGMedia(node) {
		var mediaUrl =
			...
			node.instagramConfig.credentials.user_id +
			'/media/';
		mediaUrl +=
			'?fields=id,media_type,caption,media_url,thumbnail_url,timestamp';
		mediaUrl +=
			'&access_token=' + node.instagramConfig.credentials.access_token;
	...
	RED.httpAdmin.get(
		'/instagram-credentials/auth/callback',
		function (req, res) {
			...
			if (csrfToken !== credentials.csrfToken) {
				return res
					.status(401)
					.send(RED._('instagram.error.csrf-token-mismatch'));
			}
